...
describe('IKEA overview page and price comparison tool:', function () {
    ...
    it('should navigate to OVERVIEW page and Comparison by HFBs grid will be load as default', function () {
        ...
        expect(overview_page_po_1.OverviewPage.overviewBreadCrumb().getText()).toContain('EXPLORE / Overview / Comparison By HFBs');
        utility.waitForSpinnerDisappear(common_po_1.IkeaLogin.getAppLoaderComponent());
        expect(overview_page_po_1.OverviewPage.getOverviewHeading().getText()).toEqual('COMPARISON BY HFBs');
        overview_page_po_1.OverviewPage.overviewGridColumn().then(function (columns) {
            expect(columns.length).toBe(4);
            expect(columns[0].getText()).toBe('HFBs');
            expect(columns[1].getText()).toBe('No. of Products Matched');
            expect(columns[2].getText()).toBe('No. of SKUs Where IKEA beaten');
